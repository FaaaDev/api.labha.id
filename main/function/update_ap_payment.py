...
class UpdateApPayment():
    def __init__(self, exp_id, delete):
        ...
        if delete:
            old_trans_sup = TransDdb.query.filter(and_(TransDdb.trx_code == exp.exp_code, TransDdb.trx_dbcr == "D", TransDdb.trx_desc == "JURNAL PELUNASAN %s"%(exp.exp_code))).first()
            old_trans_exp = TransDdb.query.filter(and_(TransDdb.trx_code == exp.exp_code, TransDdb.trx_dbcr == "K", TransDdb.trx_desc == "JURNAL PELUNASAN %s"%(exp.exp_code))).first()
            if old_trans_sup:
            ...
            if exp.exp_type == 1:
                ...
                old_trans_sup = TransDdb.query.filter(and_(TransDdb.trx_code == exp.exp_code, TransDdb.trx_dbcr == "D", TransDdb.trx_desc == "JURNAL PENGELUARAN %s"%(exp.exp_code))).first()
                old_trans_exp = TransDdb.query.filter(and_(TransDdb.trx_code == exp.exp_code, TransDdb.trx_dbcr == "K", TransDdb.trx_desc == "JURNAL PENGELUARAN %s"%(exp.exp_code))).all()
                if old_trans_sup:
                    db.session.delete(old_trans_sup)
                if old_trans_sup:
                ...
                # insert jurnal ap
                trans_sup = TransDdb(exp.exp_code, exp.exp_date, exp.acc, None, None,
